...
//const uint32_t RoutingProtocol::ARBITARY_MAX = 100;

std::map< uint32_t , list<StoredRrep> > RoutingProtocol::m_receivedReplies {};
/**
* \ingroup aodv
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("IsMalicious", "Flag for activating the malicous behaviour on selected nodes",
                    BooleanValue (false),  //false by default
                    MakeBooleanAccessor (&RoutingProtocol::SetMaliciousEnable,
                                        &RoutingProtocol::GetMaliciousEnable),
                    MakeBooleanChecker ())
    .AddAttribute ("enableBlackholeAttackDetection", "Flag for activating the detection of malicious nodes",
                    BooleanValue (false),  //false by default
                    MakeBooleanAccessor (&RoutingProtocol::setDetectionFlag,
                                        &RoutingProtocol::getDetectionFlag),
                    MakeBooleanChecker ())
  ;
  return tid;
}
...
/**
 * @brief 
 * Called when the node processes incoming packets 
 */
bool
      ...
      if (m_ipv4->GetInterfaceForAddress (iface.GetLocal ()) == iif)
        {
          if (dst == iface.GetBroadcast () || dst.IsBroadcast ()) 
            {
              if (m_dpd.IsDuplicate (p, header)) //discard duplicates - Stha 
                {
                  NS_LOG_DEBUG ("Duplicated packet " << p->GetUid () << " from " << origin << ". Drop.");
                  return true;
                }
              UpdateRouteLifeTime (origin, m_activeRouteTimeout); //Reverse route creating or update
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC ("Forwarding a packet");
  Ipv4Address dst = header.GetDestination ();
  Ipv4Address origin = header.GetSource ();
 ...
 */
  NS_LOG_UNCOND ("IsMalicious :"<< IsMalicious <<"\n");
  NS_LOG_UNCOND ("EnableDetection:"<< enableBlackholeAttackDetection);


  if(IsMalicious)
  {
    NS_LOG_UNCOND ("Node Configured to be malicious. Dropping packet\n");
    return true; 
  }
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  switch (tHeader.Get ())
    ...
    case AODVTYPE_RREP:
      {
        /**
         * When detection mode is on, the following should happen:
         * 1.The node should if it is the intended source node for the RREP
         * If it is, the node must check whether the path discovery timer for the corresponding destination has expired.
         * If the timer has not expired, then the source node can still receive RREPs. The destination sequence numbers in
         * each received RREP is used to calculate the Threshold for detecting malicious behaviour.
         * 2.If the node is not the intended source, then the RREP can be processed as usual.
         */

        if(enableBlackholeAttackDetection)
        {
            NS_LOG_DEBUG("============================================== DETECTION MODE =======================================");
            Ptr<Packet> packetCopy1 = packet ->Copy();
            RrepHeader rrepCopyHeader;
            packetCopy1->PeekHeader(rrepCopyHeader);

            if(IsMyOwnAddress(rrepCopyHeader.GetOrigin()))
            {
              NS_LOG_DEBUG("=============== SOURCE NODE ==================");
                Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
                Ipv4InterfaceAddress interfaceAddress = m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0);
                Ipv4Address dst = rrepCopyHeader.GetDst();
                Ipv4Address orgn = rrepCopyHeader.GetOrigin();

                // Process Hello Message
                if (dst == orgn)
                {
                  NS_LOG_DEBUG("Detection Mode:Processing Hello Message for "<< dst << " and " << orgn);
                  ProcessHello (rrepCopyHeader, receiver);
                  return;
                }
                else
                {
                  list<StoredRrep> listForDst = m_receivedReplies[dst.Get()];

                  // Check if there exist a list for this particular destination,
                  // Create or update the list by adding the packets to it.
                  NS_LOG_DEBUG ("SOURCE"<< this <<" : Collecting RREP from "  << sender << " to " << receiver);
                  NS_LOG_DEBUG ("SOURCE"<< this <<" : Size of RREP list (before) " << listForDst.size());

                  if (listForDst.empty()){  //means the list is empty
                    std::size_t sz = 1;

                    StoredRrep rrepToStore;
                    rrepToStore.setPacket(packetWithHeader);
                    rrepToStore.setReceiver(receiver);
                    rrepToStore.setSender(sender);
                    rrepToStore.setDevice(dev);
                    rrepToStore.setinterfaceAddress(interfaceAddress);
                    rrepToStore.setDstSeqNo(rrepCopyHeader.GetDstSeqno());
                    rrepToStore.setOrigin(rrepCopyHeader.GetOrigin());
                    rrepToStore.setDst(rrepToStore.getDst());

                    listForDst.assign(sz,rrepToStore);
                    m_receivedReplies[rrepCopyHeader.GetDst().Get()] = listForDst;
                    NS_LOG_DEBUG ("SOURCE"<< this <<" :Adding first packet with DstSeqNo "<< rrepCopyHeader.GetDstSeqno());
                  }
                  else
                  {
                    list<StoredRrep>::iterator itr3 = listForDst.begin();
                    advance(itr3,u_int32_t(listForDst.size())-1);
                    ...
                    listForDst.insert(itr3,rrepToStore);
                    m_receivedReplies[rrepCopyHeader.GetDst().Get()] = listForDst;

                    NS_LOG_DEBUG ("SOURCE"<< this <<" : Update the with DstSeqNo"<< rrepCopyHeader.GetDstSeqno());
                  }
              
                  NS_LOG_DEBUG ("SOURCE"<< this <<" : Size of RREP list (After) " << listForDst.size());

                  Timer::State pathDiscoveryTimerExpired = Timer::EXPIRED;
                  Timer::State actualState = m_addressReqTimer[dst].GetState();

                  if (actualState == pathDiscoveryTimerExpired)
                  {
                      int32_t avgDstSeqNo = 0;
                      int32_t variance = 0;
                      int32_t threshold = 0;

                      // Loop over the list for this particular destination and calculate the threshold
                      int count = 0;
                      int numRrep = listForDst.size();
                      list<StoredRrep>::iterator itr;
                      NS_LOG_DEBUG ("SOURCE"<< this <<" :Calculating average");
                      for(itr = listForDst.begin(); itr != listForDst.end(); ++itr)
                      {
                        StoredRrep storedPacket = *itr;

                        uint32_t seqNo = storedPacket.getDstSeqNo();
                      ...
                      avgDstSeqNo = avgDstSeqNo/count;
                      NS_LOG_DEBUG ("SOURCE"<< this <<" :avgSeqNo is "<< avgDstSeqNo);

                      NS_LOG_DEBUG ("SOURCE"<< this <<" :Calculating threshold");
                      list<StoredRrep>::iterator itr2;
                      for(itr2 = listForDst.begin(); itr2 != listForDst.end(); ++itr2)
                      {
                        StoredRrep storedPacket = *itr2;

                        uint32_t seqNo = storedPacket.getDstSeqNo();

                        uint32_t temp = pow((seqNo- avgDstSeqNo), 2);
                        variance += temp;
                      }
                      ...
                      setDstSeqThreshold(threshold);
                      NS_LOG_DEBUG ("SOURCE"<< this <<" : threshold is"<< threshold);

                      int temp2 =0;

                      NS_LOG_DEBUG ("SOURCE"<< this <<" : Processing RREPs");
                      list<StoredRrep>::iterator itr4;
                      for(itr4 = listForDst.begin(); itr4 != listForDst.end(); ++itr4)
                      {

                        StoredRrep collectedPacket = *itr4;
                        Ptr<Packet> thisPacket = collectedPacket.getPacket();

                        TypeHeader tempHeader (AODVTYPE_RREP);
                        Ptr<Packet> packetToProcess = thisPacket ->Copy();
                        packetToProcess -> PeekHeader (tempHeader);

                        Ipv4Address snd = collectedPacket.getReceiver();
                        Ipv4Address rcv = collectedPacket.getSender();
                        Ptr<NetDevice> dvc  = collectedPacket.getDev();
                        Ipv4InterfaceAddress ia = collectedPacket.getInterfaceAddress();
                        uint32_t dstSeqNo = collectedPacket.getDstSeqNo();
                        Ipv4Address origin = collectedPacket.getOrigin();
                        Ipv4Address dstn = collectedPacket.getDst();

                        temp2 =temp2 + 1;

                        RecvReplyDetection (packetToProcess,dstSeqNo,threshold,origin,dst,rcv,snd,dvc, ia);
                      }
                  }
                }
            }
            ...
            {
              NS_LOG_DEBUG ("================= INTERMEDIATE NODE ====================");
...
//============================================================================================
              ...
              */
              if(IsMalicious)
              {
                Ptr<NetDevice> device = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
                RoutingTableEntry falseToDst(device,
                                             dst,
                                             true,
...
//============================================================================================
/**
 * Setting the hop count to 1.
 * This makes the node seem as though it has the shortest path the destination
 * As per the contribitions of Shalini Satre and Mohit P.Tahiliani
 */
  if (IsMalicious)
  {
   ...
   * Disabling the RREP-ack since the algorithm being implemented does not consider it.
   */
  // if (toDst.GetHop () == 1)
  //   {
...
void 
RoutingProtocol::RecvReplyDetection (Ptr<Packet> p, uint32_t dstSeqNo, uint32_t seqNumthreshold,Ipv4Address origin ,Ipv4Address rrepDst,Ipv4Address receiver, Ipv4Address sender, Ptr<NetDevice> dvc,Ipv4InterfaceAddress interface){
  NS_LOG_DEBUG("Within the RecvReplyDetection");
  NS_LOG_FUNCTION (this << " src " << sender);
  RrepHeader rrepHeader;
  p->RemoveHeader (rrepHeader);
  Ipv4Address dst = rrepHeader.GetDst (); //The destination for which a route is supplied - stha
  NS_LOG_DEBUG ("RecvReplyDetection: RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ());
  ...
  */
  RoutingTableEntry newEntry (/*device=*/ dvc, /*dst=*/ dst, /*validSeqNo=*/ true, /*seqno=*/ dstSeqNo,
                                          /*iface=*/ interface,/*hop=*/ hop,
                                          /*nextHop=*/ sender, /*lifeTime=*/ rrepHeader.GetLifeTime ());
  
  RoutingTableEntry toDst;
  if (m_routingTable.LookupRoute (dst, toDst))
    {
        bool maliciousRrep = false;
        
        if(seqNumthreshold != 0)
        {
           maliciousRrep =  ((int32_t (dstSeqNo)) > getDstSeqThreshold());
        }
      /*
      ...
      if(maliciousRrep)
      {
        NS_LOG_DEBUG ("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< FOUND MALICIOUS RREP 1 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        NS_LOG_DEBUG ("Node Address : "<< receiver);
        NS_LOG_DEBUG ("Threshold : "<< seqNumthreshold);
        NS_LOG_DEBUG ("Source Node Dst Seq No : "<< toDst.GetSeqNo());
        NS_LOG_DEBUG ("Stored RREP seqNum : "<< dstSeqNo);
        toDst.SetUnidirectional(true); //Add the route to blacklist
        m_routingTable.Update(toDst);
        // m_routingTable.DeleteRoute(toDst.GetDestination());
      }
      else{
        NS_LOG_DEBUG ("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< FOUND NON MALICIOUS RREP >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        NS_LOG_DEBUG ("Node Address : "<< receiver);
        NS_LOG_DEBUG ("Threshold : "<< seqNumthreshold);
        NS_LOG_DEBUG ("Source Node Dst Seq No : "<< toDst.GetSeqNo());
        NS_LOG_DEBUG ("Stored RREP seqNum : "<< dstSeqNo);
        ...
        toDst.SetUnidirectional(false); //Add the route to blacklist
        m_routingTable.Update(toDst);
      ...
      else if (((int32_t (dstSeqNo) - int32_t (toDst.GetSeqNo ())) > 0 ))
        {
          m_routingTable.Update (newEntry);
        }
      else
  ...
  else
    {
     ...
     // NS_LOG_UNCOND ("add new route");
      if(dstSeqNo >= getDstSeqThreshold())
      {
        newEntry.SetUnidirectional(true);
        NS_LOG_DEBUG ("Flagged route as SUSPICIOUS");
      }
      else
      {
       m_routingTable.AddRoute (newEntry);
  ...
  if (IsMyOwnAddress (rrepHeader.GetOrigin ()))
    {
      ...
      if (m_routingTable.LookupRoute (dst, toDst))
      {
        // NS_LOG_UNCOND ("<<<<<<<<<<<<<<<<<<<<<<<<<<<< ROUTE IN THE BLACKLIST >>>>>>>>>>>>>>>>>>>>>>>>");
        m_routingTable.DeleteRoute(dst);
      }
      else
      {
        // NS_LOG_UNCOND ("<<<<<<<<<<<<<<<<<<<<<<<<<<<< ROUTE NOT IN BLACKLIST >>>>>>>>>>>>>>>>>>>>>>>>");
...
{
  NS_LOG_DEBUG("Within the RecvReply");
  NS_LOG_FUNCTION (this << " src " << sender);
  RrepHeader rrepHeader;
  p->RemoveHeader (rrepHeader);
  Ipv4Address dst = rrepHeader.GetDst (); //The destination for which a route is supplied - stha
   ...
   * -  and the destination sequence number is the Destination Sequence Number in the RREP message.
  */
  // NS_LOG_UNCOND ("RecvReply:Getting Device");
  Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
    // NS_LOG_UNCOND ("RecvReply:Creating new Entry");
  RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ dst, /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
                                          ...
                                          /*nextHop=*/ sender, /*lifeTime=*/ rrepHeader.GetLifeTime ());
  // NS_LOG_UNCOND ("About to do a route lookup");
  RoutingTableEntry toDst;
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      /*
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
   ...
   */
  // NS_LOG_UNCOND("Within the Hello Processor");
  RoutingTableEntry toNeighbor;
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor))
    {
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
      // NS_LOG_UNCOND("Getting the device");
      RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ rrepHeader.GetDst (), /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
        ...
        if(enableBlackholeAttackDetection){
          if(rrepHeader.GetDstSeqno() >= 100)
          {
            newEntry.SetUnidirectional(true); //Add node to blacklist
          }
        }
    ...
    {
      NS_LOG_DEBUG ("Updating existing route");
      ...
      if(enableBlackholeAttackDetection){
        if(rrepHeader.GetDstSeqno() >= 100)
        {
          toNeighbor.SetUnidirectional(true); //Add node to blacklist
        }
      }
      m_routingTable.Update (toNeighbor);
    }
...
/**
 * Handles the route discovery process for dst
 * First look for route in the routing table. If it does, get packet from queue and send
 *    
 */
void
