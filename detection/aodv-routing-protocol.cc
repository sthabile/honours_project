    ...
    case AODVTYPE_RREP:
      {
        ...
        if (enableBlackholeAttackDetection)
        {
          ...
          if(rrepHeader.GetDstSeqno() >=100){
             NS_LOG_UNCOND ("SOURCE"<< this <<" : =================================found malicious destination of "<<rrepHeader.GetDstSeqno() << " for "<< rrepHeader.GetDst()<< " source "<<rrepHeader.GetOrigin());
          }

          if(IsMyOwnAddress(rrepHeader.GetOrigin())) //Node is source. Must calculate Threshold and then check for malicious behavior
          {

              list<Ptr<Packet>> listForDst = m_receivedReplies[rrepHeader.GetDst()];

            // Check if there exist a list for this particular destination,
            // Create or update the list by adding the packets to it.
              NS_LOG_UNCOND ("SOURCE"<< this <<" : Collecting RREP from "  << sender << " to " << receiver);
              NS_LOG_UNCOND ("SOURCE"<< this <<" : Size of RREP list (before) " << listForDst.size());
              if (listForDst.begin() == listForDst.end()){  //means the list is empty
                std::size_t sz = 1;
                listForDst.assign(sz,packet);
                NS_LOG_UNCOND ("SOURCE"<< this <<" :Adding first packet with DstSeqNo"<< rrepHeader.GetDstSeqno());
              }
              else
              {
                list<Ptr<Packet>>::iterator itr3 = listForDst.begin();
                advance(itr3,u_int32_t(listForDst.size())-1);
                listForDst.insert(itr3,packet);
                NS_LOG_UNCOND ("SOURCE"<< this <<" : Update the with DstSeqNo"<< rrepHeader.GetDstSeqno());
              }
              
              m_receivedReplies[rrepHeader.GetDst()] = listForDst;

              NS_LOG_UNCOND ("SOURCE"<< this <<" : Size of RREP list (After) " << listForDst.size());

              Timer::State pathDiscoveryTimerExpired = Timer::EXPIRED;
              Timer::State actualState = m_addressReqTimer[rrepHeader.GetDst()].GetState();

              if(actualState == pathDiscoveryTimerExpired)
              {
                  NS_LOG_UNCOND ("SOURCE"<< this <<" :Will calculate threshold and select safest route");
                  NS_LOG_UNCOND ("SOURCE"<< this <<" :Size of RREP list (After timer expires) " << listForDst.size());

                  uint32_t avgDstSeqNo = 5;
                  uint32_t variance = 5;
                  uint32_t threshold = 5;

                  uint32_t numRrep = (uint32_t)listForDst.size();
                  if(listForDst.empty()){
                    NS_LOG_UNCOND ("SOURCE"<< this <<" :list is empty");
                  }

                  // Loop over the list for this particular destination and calculate the threshold
                  if (listForDst.begin() != listForDst.end())
                  {
                    list<Ptr<Packet>>::iterator itr;
                    for(itr = listForDst.begin(); itr != listForDst.end(); ++itr)
                    {
                      // NS_LOG_UNCOND ("SOURCE"<< this <<" :Updating average");
                      Ptr<Packet> p = *itr;
                      Ptr<Packet> packetCopy = p->Copy();
                      RrepHeader rrepCopyHeader;
                      packetCopy -> RemoveHeader(rrepCopyHeader);

                      // NS_LOG_UNCOND ("Adding " << rrepCopyHeader.GetDstSeqno()<< "to average");

                      avgDstSeqNo += rrepCopyHeader.GetDstSeqno();
                    }

                    avgDstSeqNo = avgDstSeqNo/numRrep;
                    NS_LOG_UNCOND ("SOURCE"<< this <<" ================================:avgDstSeqNo is " + to_string(avgDstSeqNo));

                    list<Ptr<Packet>>::iterator itr2;
                    for(itr2 = listForDst.begin(); itr2 != listForDst.end(); ++itr2)
                    {
                      // NS_LOG_UNCOND ("SOURCE"<< this <<" :Updating variance");
                      Ptr<Packet> p = *itr2;
                      Ptr<Packet> packetCopy = p->Copy();
                      RrepHeader rrepCopyHeader;
                      packetCopy -> RemoveHeader(rrepCopyHeader);

                      uint32_t temp = pow((rrepCopyHeader.GetDstSeqno() - avgDstSeqNo), 2);
                      variance += temp;

                      // NS_LOG_UNCOND ("Adding " << temp << "to varience");
                    }

                    variance = variance/numRrep;
                    NS_LOG_UNCOND ("SOURCE"<< this <<" ================================:variance is " + to_string(variance));
                    
                    threshold = sqrt(variance);

                    NS_LOG_UNCOND ("SOURCE"<< this <<" ================================:DstSeqNoThreshold is " + to_string(threshold));
                  ...
                  // Loop over the list and check if dstSeqNo is greater than then Threshold. Then call RecvReply.
              }
            
          }
          else  //Node is possible an intermediate node. Must process RREP as usual.
          {
              // NS_LOG_UNCOND ("INTERMEDIATE: Processing as usual");
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  NS_LOG_UNCOND ("RREP destination " << dst << " RREP origin " << rrepHeader.GetOrigin ());
...
// ==================== Should also check if the Destination Sequence Number in the RREP is less than the Dynamic Threshold - stha =============== 
...
//======================== First check if destination sequence number in rreq is less than the
      m_routingTable.AddRoute (newEntry);
    }
  // Acknowledge receipt of the RREP by sending a RREP-ACK message back
  if (rrepHeader.GetAckRequired ()) //This doesn't aid with detecting malicious behavior. The malicious node only cares about the data packets
    {
      SendReplyAck (sender);
